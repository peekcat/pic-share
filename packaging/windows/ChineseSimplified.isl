; ---------------------------------------------------------------------------
; 本文件取自 Inno Setup 源码库 jrsoftware/issrc，tag is-6_7_1：
;   Files/Languages/Unofficial/ChineseSimplified.isl
; 简体中文在 Inno Setup 6.x 里属于「非官方翻译」，不随编译器安装，
; GitHub Actions 的 windows runner（Inno Setup 6.7.1）上也没有，
; 所以必须随仓库自带。升级 Inno 大版本时记得同步更新此文件。
;
; 本地改动（仅两处，勿在升级时丢掉）：
;   1. 文件保存为 UTF-8 with BOM
;   2. LanguageCodePage 由 936 改为 0
; 原因：上游这份文件的内容是 UTF-8，却声明了 936(GBK) 码页，编译器按码页转换
; 就会得到乱码。按官方文档，「文件为 UTF-8 时 LanguageCodePage 应设为 0」。
; 语言自动检测靠 LanguageID=$0804，不受此项影响。
; ---------------------------------------------------------------------------
